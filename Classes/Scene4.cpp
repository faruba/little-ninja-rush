...
void Scene4::createScene(CCNode * bg, CCNode * fbg, CCNode * fg, RingTile * tiles) 
{
    mTiles = tiles;
    mBg = bg;
    mFront = fg;
    
    CCLayerColor *sbg = CCLayerColor::create(ccc4(201, 239, 251, 255));
    mBg->addChild(sbg);
    
    CCSprite *sky = CCSprite::createWithSpriteFrameName("cj4_bg.png");
    sky->setAnchorPoint(ccp(0, 1));
    sky->setPosition(ccp(0, SCREEN_HEIGHT));
    mBg->addChild(sky);
    ...
    for( int i=0; i<6; ++i)
    {
        mFronts[i] = CCSprite::createWithSpriteFrameName("cj4_qj.png");
    ...
    for( int i=0; i<2; ++i)
    {
        mClouds[i] = CCSprite::createWithSpriteFrameName("cj4_cloud.png");
    ...
    for( int i=0; i<6; ++i)
    {
        mBgs[i] = CCSprite::createWithSpriteFrameName("cj4_hj.png");
        mBgs[i]->setAnchorPoint(ccp(0, 1));
        mBgs[i]->setPosition(ccp(mBgs[i]->getContentSize().width*i, SCREEN_HEIGHT - 20));
        mBg->addChild(mBgs[i]);
    }
...
void Scene4::update(float delta) 
{
    ...
    //滚动背景
    {
        ...
        for( int i=0; i<6; ++i)
        {
            mBgs[i]->setPosition(ccp(-doff+dr*i, SCREEN_HEIGHT - 20));
        }
    }
