...
void SelectMenu::onEnter()
{
  ...
  {
    mTitle->setDisplayFrame(CCSpriteFrameCache::sharedSpriteFrameCache()->spriteFrameByName("classict.png"));
    mParam->setDisplayFrame(CCSpriteFrameCache::sharedSpriteFrameCache()->spriteFrameByName("xr_jz-life.png"));
  }
  else
  {
    mTitle->setDisplayFrame(CCSpriteFrameCache::sharedSpriteFrameCache()->spriteFrameByName("arcadet.png"));
    mParam->setDisplayFrame(CCSpriteFrameCache::sharedSpriteFrameCache()->spriteFrameByName("xr_jz-bouns.png"));
  }

  mProtraits[0] = CCSprite::createWithSpriteFrameName("xr_main.png");
  mProtraits[0]->setAnchorPoint(ccp(0.5f, 0));
  mProtraits[0]->setVisible(false);
  mHero->addChild(mProtraits[0]);
  mProtraits[1] = CCSprite::createWithSpriteFrameName("xr_girl.png");
  mProtraits[1]->setAnchorPoint(ccp(0.5f, 0));
  mProtraits[1]->setVisible(false);
  mHero->addChild(mProtraits[1]);
  mProtraits[2] = CCSprite::createWithSpriteFrameName("xr_musashi.png");
  mProtraits[2]->setAnchorPoint(ccp(0.5f, 0));
  mProtraits[2]->setVisible(false);
  mHero->addChild(mProtraits[2]);
  mProtraits[3] = CCSprite::createWithSpriteFrameName("xr_cat.png");
  mProtraits[3]->setAnchorPoint(ccp(0.5f, 0));
  ...
  mTargetRole = -1;
  //添加动画
  CCMoveBy *lmb1 = CCMoveBy::create(0.2f, ccp(-10, 0));
  CCMoveBy *lmb2 = CCMoveBy::create(0.2f, ccp(10, 0));
  CCSequence *lseq = (CCSequence*)CCSequence::create(lmb1, lmb2, NULL);
  CCRepeatForever *lrep = CCRepeatForever::create(lseq);
  mLeftMark->runAction(lrep);

  CCMoveBy *rmb1 = CCMoveBy::create(0.2f, ccp(10, 0));
  CCMoveBy *rmb2 = CCMoveBy::create(0.2f, ccp(-10, 0));
  CCSequence *rseq = (CCSequence*)CCSequence::create(rmb1, rmb2, NULL);
  CCRepeatForever *rrep = CCRepeatForever::create(rseq);
...
void SelectMenu::updateCharacterInfo(int rid)
{
  if( GamePlay::sharedGamePlay()->getGameMode() == MODE_CLASSIC )
  {
    ...
    for(int i=0; i<GameData::roleCurrHP(rid); ++i)
    {
      ...
      heart->setPosition(ccp(i*(1+heart->getContentSize().width), 0));
  ...
  {
    mBouns->setDisplayFrame(CCSpriteFrameCache::sharedSpriteFrameCache()->spriteFrameByName(gBouns[rid]));
  ...
  {
    CCSprite *dart = CCSprite::createWithSpriteFrameName("ui-dart.png");
    dart->setPosition(ccp(i*dartoff, 0));
  ...
  mEquipDart->setDisplayFrame(CCSprite::create(sh->icon->getCString())->displayFrame());
  mEquipBlade->setDisplayFrame(CCSprite::create(ka->icon->getCString())->displayFrame());
  mEquipSpecial->setDisplayFrame(CCSprite::create(sp->icon->getCString())->displayFrame());

  //更新人物显示
  switch (rid) {
    case 0:
      mDesc->setDisplayFrame(CCSpriteFrameCache::sharedSpriteFrameCache()->spriteFrameByName("xr_kt.png"));
      mBtnStart->setNormalImage(CCSprite::createWithSpriteFrameName("xr_st1.png"));
      mBtnStart->setSelectedImage(CCSprite::createWithSpriteFrameName("xr_st1.png"));
      break;
    case 1:
      mDesc->setDisplayFrame(CCSpriteFrameCache::sharedSpriteFrameCache()->spriteFrameByName("xr_sr.png"));
      mBtnStart->setNormalImage(CCSprite::createWithSpriteFrameName("xr_st2.png"));
      mBtnStart->setSelectedImage(CCSprite::createWithSpriteFrameName("xr_st2.png"));
      break;
    case 2:
      mDesc->setDisplayFrame(CCSpriteFrameCache::sharedSpriteFrameCache()->spriteFrameByName("xr_ms.png"));
      mBtnStart->setNormalImage(CCSprite::createWithSpriteFrameName("xr_st3.png"));
      mBtnStart->setSelectedImage(CCSprite::createWithSpriteFrameName("xr_st3.png"));
      break;
    case 3:
      mDesc->setDisplayFrame(CCSpriteFrameCache::sharedSpriteFrameCache()->spriteFrameByName("xr_mr.png"));
      mBtnStart->setNormalImage(CCSprite::createWithSpriteFrameName("xr_st4.png"));
      mBtnStart->setSelectedImage(CCSprite::createWithSpriteFrameName("xr_st4.png"));
      break;
  }
  ...
  mPreview->setAnchorPoint(ccp(0.4f, 0.0625f));
  mPreview->setPosition(ccp( -10, 10));
  ...
  if( GameRecord::sharedGameRecord()->char_contract[rid] == 0 )
  {
    ...
    CCSprite *hire = CCSprite::createWithSpriteFrameName("hire.png");
  ...
  mHeroDesc->setDisplayFrame(CCSpriteFrameCache::sharedSpriteFrameCache()->spriteFrameByName(gHeroDesc[rid]));
}
...
void SelectMenu::updateAngle()
{
  for(int i=0; i<GAME_CHARCOUNT; ++i)
  {
    ...
    if( disangle > 89 && disangle < 270 )
    {
      ...
      mProtraits[i]->setPosition(ccp(pos, 0));
...
void SelectMenu::update(float delta)
{
...

  {
    ...
    CCPoint mp = mMoney->getPosition();
    mp.x = 450 - 7.15f*strlen(mMoney->getString());
    mMoney->setPosition( mp );
    CCPoint cm = mCoinMark->getPosition();
    cm.x = mp.x - 10;
...
void SelectMenu::onStart()
{
    {
  {
    ...
    if( GameRecord::sharedGameRecord()->coins >= cost )
    {
      ...
      SimpleAudioEngine::sharedEngine()->playEffect(CCFileUtils::sharedFileUtils()->fullPathForFilename("sound/menu-start.mp3").c_str());
...
void SelectMenu::setSceneOutro(CCScene* newscene)
{
  if( mIntroFlag )
  {
    return;
...
SEL_MenuHandler SelectMenu::onResolveCCBCCMenuItemSelector(CCObject * pTarget, const char* pSelectorName)
{
...
SEL_CCControlHandler SelectMenu::onResolveCCBCCControlSelector(CCObject * pTarget, const char* pSelectorName)
{
  CCLog("Control");
  return NULL;
}
bool SelectMenu::onAssignCCBMemberVariable(CCObject* pTarget, const char* pMemberVariableName, CCNode* pNode)
{
